...
for dataset_name in [args.dataset_name]:
    ...
    for model_name in [args.model_name]:
        ...
        for idx in tqdm(run_indices):
            ...
            print(new_data)
            with open(f"temp_{args.dataset_name}_{args.model_name.split('/')[-1]}_{args.score_function}.json", "w") as fout_temp:
                json.dump(new_data, fout_temp)

            path = Path(f"temp_{args.dataset_name}_{args.model_name.split('/')[-1]}_{args.score_function}.json")
